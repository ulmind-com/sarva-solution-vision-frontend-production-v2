...
import { Info, TrendingUp, Calendar, ArrowRight, ArrowLeft, HeartPulse } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { getHealthEducationBonusStatus, getHealthEducationBonusHistory, getHealthEducationBonusLiveEstimate } from "@/services/userService";

export default function HealthEducationBonus() {
    ...
    const fetchData = async (page: number) => {
        ...
        try {
            const [statusRes, historyRes, estimateRes] = await Promise.allSettled([
                getHealthEducationBonusStatus(),
                getHealthEducationBonusHistory(page, 10),
                getHealthEducationBonusLiveEstimate()
            ]);
            ...
            console.error("Failed to fetch Health & Education Bonus data:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6 lg:p-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Health & Education Bonus</h1>
                    <p className="text-muted-foreground mt-1">
                        Track your 5% BV Pool earnings — <strong>50,000 BV = 1 Unit</strong>, <strong>No Capping</strong>
                    </p>
                </div>
                <Badge variant="outline" className="text-sm border-rose-500/30 text-rose-600 bg-rose-500/10 px-3 py-1.5 self-start">
                    <HeartPulse className="h-3.5 w-3.5 mr-1" /> 5% Pool · No Cap
                </Badge>
            </div>
            ...
            {liveEstimate && (
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <Card className="glass premium-shadow border-rose-500/10 bg-rose-500/5">
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-medium text-rose-600">5% Pool Value</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-rose-600">₹{liveEstimate.companyBV?.poolAmount?.toLocaleString('en-IN', { maximumFractionDigits: 2 }) || 0}</div>
                            </CardContent>
                        </Card>
                        <Card className="glass premium-shadow border-primary/10">
                            <CardHeader className="pb-2">
                        ...
                        <Card className="glass premium-shadow border-green-500/30 bg-green-500/10">
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-green-700/80 mt-1">Based on {liveEstimate.myEstimate?.myUnits || 0} units (50,000 BV each)</p>
                            </CardContent>
                        </Card>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <Card className="glass premium-shadow border-rose-500/20 bg-rose-500/5">
                                <CardContent className="pt-6">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="text-sm font-medium text-rose-600 mb-1">Estimated Units</p>
                                    ...
                                    <p className="text-xs text-rose-600/80 mt-3">
                                        50,000 BV = 1 Unit · Unlimited earning potential
                                    </p>
                                </CardContent>
                            </Card>
                        </div>
            ...
            <Card className="glass premium-shadow overflow-hidden">
                <CardHeader className="border-b border-border/50 bg-muted/20 pb-4">
                    ...
                    <CardDescription>Records of your distributed units and health & education bonus payouts.</CardDescription>
                </CardHeader>
                                ...
                                {historyData.length > 0 ? (
                                    historyData.map((record, index) => (
                                        <TableRow key={index} className="hover:bg-accent/40 transition-colors">
                                            ...
                                            <TableCell>
                                                <Badge variant="outline" className="border-rose-500/20 bg-rose-500/5 text-rose-600">
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={6} className="h-32 text-center text-muted-foreground">
                                            No health & education bonus payout history found.
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
