...
import { Info, TrendingUp, Calendar, ArrowRight, ArrowLeft, Zap } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { getStartupBonusStatus, getStartupBonusHistory, getStartupBonusLiveEstimate } from "@/services/userService";

export default function StartupBonus() {
    ...
    const fetchData = async (page: number) => {
        ...
        try {
            const [statusRes, historyRes, estimateRes] = await Promise.allSettled([
                getStartupBonusStatus(),
                getStartupBonusHistory(page, 10),
                getStartupBonusLiveEstimate()
            ]);
            ...
            console.error("Failed to fetch Startup Bonus data:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6 lg:p-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Start Up Bonus</h1>
                    <p className="text-muted-foreground mt-1">
                        Track your 18% BV Pool earnings — <strong>5000 BV = 1 Unit</strong>, <strong>No Capping</strong>
                    </p>
                </div>
                <Badge variant="outline" className="text-sm border-emerald-500/30 text-emerald-600 bg-emerald-500/10 px-3 py-1.5 self-start">
                    <Zap className="h-3.5 w-3.5 mr-1" /> No Unit Cap
                </Badge>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <Card className="glass premium-shadow border-green-500/30 bg-green-500/10">
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-green-700/80 mt-1">Based on {liveEstimate.myEstimate?.myUnits || 0} units (5000 BV each)</p>
                            </CardContent>
                        </Card>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            {/* Estimated Units Card — No Capping */}
                            <Card className="glass premium-shadow border-emerald-500/20 bg-emerald-500/5">
                                <CardContent className="pt-6">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="text-sm font-medium text-emerald-600 mb-1">Estimated Units</p>
                                    ...
                                    <p className="text-xs text-emerald-600/80 mt-3">
                                        5000 BV = 1 Unit · Unlimited earning potential
                                    </p>
                                </CardContent>
                            </Card>
                        </div>
            ...
            <Card className="glass premium-shadow overflow-hidden">
                <CardHeader className="border-b border-border/50 bg-muted/20 pb-4">
                    ...
                    <CardDescription>Records of your distributed units and startup bonus payouts.</CardDescription>
                </CardHeader>
                                ...
                                {historyData.length > 0 ? (
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={6} className="h-32 text-center text-muted-foreground">
                                            No startup bonus payout history found.
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
